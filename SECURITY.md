...
- **Receipt verification tokens** — Each receipt gets a random 48-char `verification_token`. Public `GET /api/v1/verify-receipt/{token}` (throttled) returns limited fields (number, amount, customer name, branch) — not full payment internals. PDFs/HTML stay on the private disk; authenticated users need `receipts.view`.
