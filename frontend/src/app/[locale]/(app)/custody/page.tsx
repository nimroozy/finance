...
import { listCustody, type CustodyRecord } from "@/lib/inventory";
import {
  BranchPicker,
  EmptyWorkspace,
  ErrorWorkspace,
...
export default function CustodyPage() {
  ...
  const [rows, setRows] = useState<CustodyRecord[]>([]);
  ...
  const columns: DataTableColumn<CustodyRecord>[] = [
    { key: "id", label: t("columns.id"), render: (row) => String(row.id) },
    { key: "user", label: t("columns.user"), render: (row) => row.user?.name || String(row.user_id) },
    { key: "product", label: t("columns.product"), render: (row) => row.product?.name_en || "—" },
    { key: "status", label: t("columns.status"), render: (row) => <StatusBadge status={row.status || "open"} /> },
  ];

  return (
    <div className="space-y-4" data-testid="custody-list">
      <WorkspaceHeader title={t("custodyTitle")} subtitle={t("custodySubtitle")} />
      <FilterBar
      ...
      {!loading && !error && rows.length === 0 ? <EmptyWorkspace label={t("emptyCustody")} /> : null}
          ...
          <div className="space-y-3 md:hidden">
            {rows.map((row) => (
              <MobileRecordCard key={row.id}
                title={row.user?.name || `#${row.id}`}
                subtitle={row.product?.name_en}
                badges={<StatusBadge status={row.status || "open"} />}
              />
            ))}
          </div>
