...
import { listMaintenancePlans, type MaintenancePlan } from "@/lib/inventory";
import {
  BranchPicker,
  EmptyWorkspace,
  ErrorWorkspace,
...
export default function MaintenancePage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await listMaintenancePlans({
        page: pageNum,
        per_page: 15,
  ...
  const columns: DataTableColumn<MaintenancePlan>[] = [
    ...
    { key: "frequency", label: t("fields.frequency"), render: (row) => row.frequency || "—" },
    { key: "due", label: t("fields.nextDue"), render: (row) => row.next_due_date || "—" },
  ];

  return (
    <div className="space-y-4" data-testid="maintenance-list">
      <WorkspaceHeader title={t("maintenanceTitle")} subtitle={t("maintenanceSubtitle")} />
      <FilterBar
      ...
      {!loading && !error && rows.length === 0 ? <EmptyWorkspace label={t("emptyMaintenance")} /> : null}
