...
import { listPurchaseRequests, type PurchaseRequest } from "@/lib/inventory";
import {
  BranchPicker,
  EmptyWorkspace,
  ErrorWorkspace,
...
export default function PurchaseRequestsPage() {
  ...
  const columns: DataTableColumn<PurchaseRequest>[] = [
    { key: "number", label: t("columns.requestNumber"), render: (row) => row.request_number || `#${row.id}` },
  ...
  return (
    <div className="space-y-4" data-testid="purchase-requests">
      <WorkspaceHeader title={t("purchaseRequestsTitle")} subtitle={t("purchaseRequestsSubtitle")} />
      <FilterBar
      ...
      {!loading && !error && rows.length === 0 ? <EmptyWorkspace label={t("emptyPurchaseRequests")} /> : null}
