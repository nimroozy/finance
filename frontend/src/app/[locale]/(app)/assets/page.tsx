...
import { listFixedAssets, type FixedAsset } from "@/lib/inventory";
import {
  BranchPicker,
  EmptyWorkspace,
  ErrorWorkspace,
...
export default function AssetsListPage() {
  ...
  const columns: DataTableColumn<FixedAsset>[] = [
    { key: "number", label: t("columns.assetNumber"), render: (row) => (
      <Link href={`/assets/${row.id}`} className="font-medium text-primary hover:underline">{row.asset_number}</Link>
    )},
  ...
  return (
    <div className="space-y-4" data-testid="assets-list">
      <WorkspaceHeader title={t("assetsTitle")} subtitle={t("assetsSubtitle")} />
      <FilterBar
      ...
      {!loading && !error && rows.length === 0 ? <EmptyWorkspace label={t("emptyAssets")} /> : null}
