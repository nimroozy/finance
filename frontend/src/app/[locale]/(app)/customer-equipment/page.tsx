...
export default function CustomerEquipmentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await listCustomerEquipment({
        page: pageNum,
        per_page: 15,
  ...
  const columns: DataTableColumn<CustomerEquipment>[] = [
    { key: "id", label: t("columns.id"), render: (row) => String(row.id) },
    { key: "product", label: t("columns.product"), render: (row) => row.product?.name_en || "—" },
    { key: "serial", label: t("columns.serial"), render: (row) => row.serial_number || row.equipment?.serial_number || "—" },
    { key: "status", label: t("columns.status"), render: (row) => <StatusBadge status={row.status || "installed"} /> },
  ];

  return (
    <div className="space-y-4" data-testid="customer-equipment">
      <WorkspaceHeader title={t("customerEquipmentTitle")} subtitle={t("customerEquipmentSubtitle")} />
      <FilterBar
      ...
      {!loading && !error && rows.length === 0 ? <EmptyWorkspace label={t("emptyCustomerEquipment")} /> : null}
          ...
          <div className="space-y-3 md:hidden">
            {rows.map((row) => (
              <MobileRecordCard key={row.id}
                title={row.product?.name_en || `#${row.id}`}
                subtitle={row.serial_number || row.equipment?.serial_number || ""}
                badges={<StatusBadge status={row.status || "installed"} />}
              />
            ))}
          </div>
