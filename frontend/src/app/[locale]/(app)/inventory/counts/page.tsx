...
import { listStockCounts, type StockCount } from "@/lib/inventory";
import {
  BranchPicker,
  EmptyWorkspace,
  ErrorWorkspace,
...
export default function CountsListPage() {
  ...
  const columns: DataTableColumn<StockCount>[] = [
    {
      key: "number",
      label: t("columns.countNumber"),
      render: (row) => (
        <Link href={`/inventory/counts/${row.id}`} className="font-medium text-primary hover:underline">
          {row.count_number || `#${row.id}`}
        </Link>
      ),
    },
    { key: "status", label: t("columns.status"), render: (row) => <StatusBadge status={row.status} /> },
    { key: "location", label: t("columns.location"), render: (row) => String(row.location_id) },
  ];

  return (
    <div className="space-y-4" data-testid="inventory-counts">
      <WorkspaceHeader
        title={t("countsTitle")}
      ...
      {!loading && !error && rows.length === 0 ? <EmptyWorkspace label={t("emptyCounts")} /> : null}
