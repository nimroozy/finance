...
export default function ReservationsListPage() {
  ...
  async function onCreate() {
    ...
    if (!createBranch || !createLocation || !createProduct) {
    ...
    try {
      const res = await createReservation({
        ...
        location_id: Number(createLocation),
        items: [{ product_id: Number(createProduct), qty: Number(createQty) || 1 }],
      });
      setShowCreate(false);
      router.push(`/inventory/reservations/${res.data.id}`);
    } catch (err) {
  ...
  const columns: DataTableColumn<Reservation>[] = [
    {
      key: "number",
      label: t("columns.reservationNumber"),
      render: (row) => (
        <Link href={`/inventory/reservations/${row.id}`} className="font-medium text-primary hover:underline">
          {row.reservation_number}
        </Link>
      ),
    },
    { key: "status", label: t("columns.status"), render: (row) => <StatusBadge status={row.status} /> },
    { key: "items", label: t("columns.items"), render: (row) => String(row.items?.length ?? 0) },
  ];

  return (
    <div className="space-y-4" data-testid="inventory-reservations">
      <WorkspaceHeader
        title={t("reservationsTitle")}
        subtitle={t("reservationsSubtitle")}
        actions={
          canManage ? (
            <Button data-testid="new-reservation" onClick={() => setShowCreate((v) => !v)}>
              {t("newReservation")}
            </Button>
          ) : null
        }
      />
      {showCreate ? (
        <div data-testid="create-reservation-form"><Panel className="space-y-3 p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            />
            <Select value={createLocation} onChange={(e) => setCreateLocation(e.target.value)} data-testid="reservation-location">
              <option value="">{t("fields.selectLocation")}</option>
              {locations.map((loc) => (
                <option key={loc.id} value={loc.id}>{loc.name}</option>
              ))}
            </Select>
            <Select value={createProduct} onChange={(e) => setCreateProduct(e.target.value)} data-testid="reservation-product">
              <option value="">{t("fields.selectProduct")}</option>
              {products.map((p) => (
                <option key={p.id} value={p.id}>{p.name_en}</option>
              ))}
            </Select>
            <Input type="number" value={createQty} onChange={(e) => setCreateQty(e.target.value)} data-testid="reservation-qty" />
          </div>
          <Button data-testid="save-reservation" disabled={createBusy} onClick={() => void onCreate()}>
            {tCommon("create")}
          </Button>
        </Panel></div>
      ) : null}
      ...
      {!loading && !error && rows.length === 0 ? <EmptyWorkspace label={t("emptyReservations")} /> : null}
