...
class CashboxTransferService
{
    ...
    {
        if (! in_array($type, ['cashbox_transfer', 'bank_deposit'], true)) {
            throw new InvalidArgumentException('Unsupported transfer type.');
        }
        if (! Money::isPositive($amount) || ($type === 'cashbox_transfer' && ! $to)) {
            throw new InvalidArgumentException('Valid transfer amount and destination are required.');
        }
        if ($to && $to->id === $from->id) {
            throw new InvalidArgumentException('Source and destination cashboxes must differ.');
        }
    ...
    public function submit(BranchCashboxTransfer $transfer, User $actor): BranchCashboxTransfer
    {
        return $this->change($transfer, 'draft', 'submitted', $actor, 'submitted_at');
    }

    public function approve(BranchCashboxTransfer $transfer, User $actor): BranchCashboxTransfer
    {
        return DB::transaction(function () use ($transfer, $actor) {
            $t = $this->change($transfer, 'submitted', 'approved', $actor, 'approved_at');
            $t->approved_by = $actor->id;
            $t->save();
            DB::table('branch_cashbox_transfer_approvals')->insert([
                'transfer_id' => $t->id,
    ...
    public function send(BranchCashboxTransfer $transfer, User $actor): BranchCashboxTransfer
    {
        return DB::transaction(function () use ($transfer, $actor) {
            ...
            if ($locked->status !== 'approved') {
                throw new InvalidArgumentException('Only approved transfers can be sent.');
            }
    ...
    public function receive(BranchCashboxTransfer $transfer, User $actor): BranchCashboxTransfer
    {
        return DB::transaction(function () use ($transfer, $actor) {
            ...
            if ($locked->status !== 'sent') {
                throw new InvalidArgumentException('Only sent transfers can be received.');
            }
            if ($locked->type === 'bank_deposit') {
    ...
    public function reverse(BranchCashboxTransfer $transfer, User $actor, string $reason): BranchCashboxTransfer
    {
        return DB::transaction(function () use ($transfer, $actor, $reason) {
            $locked = BranchCashboxTransfer::withoutGlobalScopes()->whereKey($transfer->id)->lockForUpdate()->firstOrFail();
            if (! in_array($locked->status, ['approved', 'sent', 'received'], true)) {
                throw new InvalidArgumentException('Only approved, sent, or received transfers can be reversed.');
            }
            if ($locked->status === 'received' && $locked->type === 'cashbox_transfer') {
                $this->cashboxes->debit(BranchCashbox::withoutGlobalScopes()->findOrFail($locked->to_cashbox_id), $locked->amount, 'cashbox_transfer_reversal_debit', $actor, $reason);
            }
            if (in_array($locked->status, ['sent', 'received'], true)) {
                $this->cashboxes->credit(BranchCashbox::withoutGlobalScopes()->findOrFail($locked->from_cashbox_id), $locked->amount, 'cashbox_transfer_reversal', null, $actor, $reason);
            }
            DB::table('branch_cashbox_transfer_reversals')->insert([
    ...
    {
        return DB::transaction(function () use ($transfer, $from, $to, $at) {
            $locked = BranchCashboxTransfer::withoutGlobalScopes()->whereKey($transfer->id)->lockForUpdate()->firstOrFail();
            if ($locked->status !== $from) {
                throw new InvalidArgumentException("Transfer must be {$from}.");
            }
            $locked->status = $to;
            $locked->{$at} = now();
            $locked->save();
